...
class DocanDeyeEMSPanel extends HTMLElement {
 ...
 render(){
  ...
  this.text('plan-mode',d.physical_authority?'LIVE CONTROLLER':plan.mode==='production_shadow'?'PRODUCTION POLICY · SHADOW':plan.mode==='observed'?'OBSERVED CONTROLLER PLAN · READ ONLY':plan.mode==='forecast_shadow'?'FORECAST SIMULATION · NOT ARMED':'SHADOW ESTIMATE · NOT ARMED');
  this.text('plan-status',String(plan.status||'unavailable').replaceAll('_',' ')+(plan.mode==='production_shadow'&&plan.reason?' · '+plan.reason:''));
