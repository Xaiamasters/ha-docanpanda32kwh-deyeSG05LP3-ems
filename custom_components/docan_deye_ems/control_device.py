"""Bounded Deye control transport, presently restricted to loopback laboratory use.

Register facts and validation status are recorded in CONTROL_ADAPTER.md. No
caller can supply a register address to write. Physical writes remain disabled.
"""
...
READABLE = frozenset(r for first, count in READ_BLOCKS for r in range(first, first + count))
...
class DeyeDevice:
    ...
    async def _request(self, function, address, values, guard=None):
        if function == 3:
        ...
        if function == 16 and not ipaddress.ip_address(self.connection['host']).is_loopback:
            raise WriteDenied('physical_commissioning_not_available')
        writer = None
        lease = None
        sent = False
        try:
            ...
            # The transaction manager journals intent before calling this method.
            # Cancellation does not imply that the command failed to reach a device.
            raise
        except (OSError, TimeoutError, asyncio.IncompleteReadError, DeviceError) as exc:
            if function == 16 and sent:
                raise UncertainWrite('write_outcome_unknown') from None
            if isinstance(exc, WriteDenied):
                raise
    ...
    async def write(self, field, value, *, authority, generation, expected=None, stop_only=False):
        ...
        await self._request(16, spec.register, (encoded,), guard)
