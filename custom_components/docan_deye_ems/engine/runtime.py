...
def validate_pin(plan):
    ...
    for group in plan.get('export_clusters',[]):
        ...
        if not 0<=start<end<=count or not 25<=floor<=100:raise ValueError('invalid_controller_plan')
...
class ShadowRuntime:
    ...
    def __init__(self,storage,contract=None,limits=None):
    ...
    def _run(self,snapshot,at,prices,tomorrow,observed_plan,draws):
        ...
        if len(prices)!=count or not all(finite(x) for x in prices):raise ValueError('incomplete_published_price_day')
        if tomorrow is not None and (len(tomorrow) not in ((92,94,96,98,100) if hasattr(tomorrow,'contract') else (96,)) or not all(finite(x) for x in tomorrow)):raise ValueError('invalid_tomorrow_prices')
        state.update(at=at.isoformat(),prices=prices,prices_tomorrow=tomorrow)
        ...
        if self.contract.household_profile and observed is None:
            from .schedule import update_schedule
            pin=update_schedule(self.storage,at,prices,tomorrow,state['soc'],self.contract)
            schedule_error=saved.get('schedule_pending',{}).get('reason')
        else:
        ...
        policy=Controller(at,transport,self.storage,pin,self.contract,limits=self.limits,allow_writes=False)
        ...
        if pin:
            ...
            for start,end,floor in pin.get('export_clusters',[]):
                windows.append({'start':(prices.instant(start) if hasattr(prices,'instant') else midnight+timedelta(minutes=start*15)).isoformat(),
                                'end':(prices.instant(end) if hasattr(prices,'instant') else midnight+timedelta(minutes=end*15)).isoformat(),'action':'export','reserve_soc':floor})
            # The charge window displayed is the observed firmware window, not a
            # claim that an observed external program was applied by this engine.
            programs=({int(i):row for i,row in pin['programs'].items()}
                      if self.contract.household_profile and pin.get('programs') else state['programs'])
            for i in range(1,7):
                ...
                windows.append({'start':a.isoformat(),'end':b.isoformat(),'action':'charge',
                                'source':'planned' if self.contract.household_profile else 'observed'})
        ...
        return {'mode':'production_shadow','status':outcome.get('action','no_complete_target'),
                ...
                'reserve_basis':(pin or {}).get('reserve_basis','supplied_plan'),'schedule_error':schedule_error,'pinned_plan_present':pin is not None,
                'program_status':(pin or {}).get('program_status','preview'),
                'expected_export_kwh':(pin or {}).get('expected_export_kwh'),
                'expected_export_eur':(pin or {}).get('expected_export_eur'),
                'assumptions':['Planning preview. Live control requires separate commissioning.',
                               'The charge window is the proposed firmware schedule.' if self.contract.household_profile else 'The charge window comes from observed firmware settings.',
