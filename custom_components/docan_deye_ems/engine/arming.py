...
def schema_v2_extend(payload: dict, prices, k) -> dict:
    ...
    payload['clusters_detail'] = [{'start': writer.slot_to_hhmm(a), 'end': writer.slot_to_hhmm(b), 'price_ct': round(sum((prices[i] for i in range(a, b))) / (b - a) * 100, 2)} for a, b, _fl in payload.get('export_clusters') or []]
    ...
    if cw and cw[0] is not None and (soc0 < target):
        need_flat = max(0.0, min(target, ACCEPT_FLAT_TO_PCT) - soc0) * PACK_KWH_PER_PCT
        need_taper = max(0.0, target - max(soc0, ACCEPT_FLAT_TO_PCT)) * PACK_KWH_PER_PCT
        hours = need_flat / ACCEPT_KW_FLAT + need_taper / ACCEPT_KW_TAPER
        done_slot = min(cw[1], cw[0] + int(hours * 4 + 0.999))
        span = range(cw[0], max(cw[0] + 1, done_slot))
        payload['charge_block'] = {'start': writer.slot_to_hhmm(cw[0]), 'end': writer.slot_to_hhmm(cw[1]), 'avg_price_ct': round(sum((prices[i] for i in span)) / len(span) * 100, 2), 'est_kwh': round(need_flat + need_taper, 1), 'est_complete_time': writer.slot_to_hhmm(done_slot), 'estimate_basis': f'{ACCEPT_KW_FLAT} kW flat to {ACCEPT_FLAT_TO_PCT:.0f}%, ~{ACCEPT_KW_TAPER} kW taper mean; selection-time prices'}
    clusters = payload.get('export_clusters') or []
    floor = min((fl for _a, _b, fl in clusters), default=None)
    slot_kwh = 0.25 * EXPORT_KW_METER
    slots = sorted((prices[i] for a, b, _fl in clusters for i in range(a, b)), reverse=True)
    window_hours = sum((b - a for a, b, _fl in clusters)) * 0.25
    budget_kwh = None
    if floor is not None and soc0 is not None:
        sell_open = max(soc0, target)
        budget_kwh = floor_budget_kwh(sell_open - floor, window_hours)
    ...
    payload['expected_export_basis'] = f"min(window {round(window_kwh, 1)} kWh, floor budget {('n/a' if budget_kwh is None else round(budget_kwh, 1))} kWh) at {EXPORT_KW_METER} kW meter (P1-measured, 9 evenings 2-10 Aug), priciest slots first; budget is EXPORT-ONLY = pts x {DISCHARGE_KWH_PER_PCT} x {PACK_TO_METER} - {HOUSE_KW_IN_WINDOW} kW x {round(window_hours, 2)} h house draw (F39)"
    payload['window_kwh'] = round(window_kwh, 2)
    payload['floor_budget_kwh'] = None if budget_kwh is None else round(budget_kwh, 2)
    return payload

def floor_budget_kwh(points: float, window_hours: float) -> float:
...
def choose_ceiling(prices, soc0, k, plan_base, plan_trial):
    ...
    sell = _mean(prices, [i for p in export_runs for i in range(p.start_slot, p.end_slot)])
    if sell is None:
    ...
    window_kwh = window_hours * EXPORT_KW_METER
    base_budget = floor_budget_kwh(CEILING_BASE - k.reserve_floor_pct, window_hours)
    ...
    return (CEILING_TRIAL, f'ceiling {CEILING_TRIAL:.0f} (deadman {DEADMAN_SOC_MAX:.0f}, above it): (a) planner reaches {CEILING_TRIAL:.0f}% inside the cheap block {how}; (b) {money}; (c) evening is floor-bound with {unsold:.1f} kWh of window unsold')
